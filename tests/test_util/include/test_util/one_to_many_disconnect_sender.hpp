#ifndef _TEST_UTIL_ONE_TO_MANY_DISCONNECT_SENDER
#define _TEST_UTIL_ONE_TO_MANY_DISCONNECT_SENDER
...
int one_to_many_disconnect_sender(S tx, R rx) {
  ...
  for (int i = 0; i < 10000; ++i) {
    if (item_counts[i] != 1) {
      std::cerr << "wrong item count for item " << i << ": expected 1 got "
                << item_counts[i];
      return 1;
    }
  }
